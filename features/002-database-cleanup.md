...
## Key Changes
*   `senpuki/backend/base.py`: Added `cleanup_executions` method signature to the `Backend` protocol.
*   `senpuki/backend/sqlite.py`: Implemented the SQL logic for `cleanup_executions` to delete old records based on their state and `completed_at` timestamp.
*   `senpuki/executor.py`: Integrated a background `_cleanup_loop` into the `serve` function, responsible for periodically invoking the `cleanup_executions` method.
*   `senpuki/utils/async_sqlite.py`: Added `rowcount` property to `AsyncCursor` to support the cleanup logic, allowing the backend to report the number of deleted records.

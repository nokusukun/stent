...
*   **`stent/backend/sqlite.py`**: 
    *   Switched import to use the installed `aiosqlite` library instead of the custom `stent.utils.async_sqlite` implementation (which was causing segfaults during tests).

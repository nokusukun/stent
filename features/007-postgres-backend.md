...
## Key Changes
*   **`senpuki/backend/postgres.py`**: Implemented `PostgresBackend` with `asyncpg`.
...
*   **`senpuki/executor.py`**: Added `PostgresBackend` to `Backends` helper.
*   **`tests/utils.py`**: Added `get_test_backend` and `cleanup_test_backend` to support `SENPUKI_TEST_BACKEND=postgres`.
...
```python
executor = Senpuki(backend=Senpuki.backends.PostgresBackend("postgres://user:pass@localhost:5432/db"))
```

**Running Tests with Postgres:**
```bash
# Ensure Postgres is running (e.g., docker run -p 5432:5432 -e POSTGRES_PASSWORD=postgres postgres)
export SENPUKI_TEST_BACKEND=postgres
export SENPUKI_TEST_PG_DSN="postgres://postgres:postgres@localhost:5432/senpuki_test"
uv run pytest
```
